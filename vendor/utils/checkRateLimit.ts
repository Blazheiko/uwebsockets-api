import { HttpData, RateLimit, ResponseData, RouteItem } from '../types/types.js';
...
function determineRateLimit(route: RouteItem, groupRateLimit?: RateLimit): RateLimit | null {
...
export default async function checkRateLimit(
    ...
    try {
        ...
        responseData.setHeader('X-RateLimit-Limit', rateLimit.maxRequests.toString());
        responseData.setHeader('X-RateLimit-Remaining', Math.max(0, rateLimit.maxRequests - rateLimitInfo.requests).toString());
        responseData.setHeader('X-RateLimit-Reset', Math.ceil(rateLimitInfo.resetTime / 1000).toString());
        ...
        if (rateLimitInfo.requests > rateLimit.maxRequests) {
            ...
            responseData.setHeader('Retry-After', Math.ceil((rateLimitInfo.resetTime - Date.now()) / 1000).toString());
